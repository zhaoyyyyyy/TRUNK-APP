...
public class ResourceController extends BaseController<Resource> {
	...
	public String list(@ModelAttribute JQGridPage<Resource> page,@ApiIgnore ResourceVo resourceVo,String cols) {
		//zn
	...
	public String save(Resource resource) {
		...
		if(StringUtils.isNotBlank(id)){
			...
			Resource resourceBy = resourceService.getOrgByOrgCode(resource.getResourceCode());
			if(resourceBy!=null&&!oldResource.getResourceCode().equals(resource.getResourceCode())){
				return "haveSameCode";
			}
	...
	public Map<String,Object> initBackground() {
		...
		for(int i=0;i<resourceList.size();i++){
			...
			List<Resource> re1=resourceService.getParentResourceByRole(user, resourceList.get(i).getId());
		...
		map.put("user", user);
		map.put("date",DateUtil.formatDate(new Date(),DateUtil.FMT_DATE_YYYY_MM_DD_EEEE));
		return map;
	}
...
//			}
		...
		return this.getTree(organization, "true", null, sb, roleId,list);
	}
	...
	private String getTree(Resource common, String selectable, String treeType,
		...
		if (list != null && list.size() > 0 && list.contains(common.getId())) {
			...
			getSubTree(common.getChildren(), selectable, treeType, htmlCon,
					roleId, list);
	...
	private String getSubTree(Set<Resource> set, String selectable,
			String treeType, StringBuffer htmlCon, String roleId,
			List<String> list) {
	...
	private String getLeaf(Resource common, String treeType,
			StringBuffer htmlCon, String roleId, List<String> list) {
		if (list.contains(common.getId())) {
			String orgType = common.getType();
			htmlCon.append("<li id='").append(common.getId()).append(
					"' name='").append(common.getResourceCode()).append(
					"' selectable='true'");

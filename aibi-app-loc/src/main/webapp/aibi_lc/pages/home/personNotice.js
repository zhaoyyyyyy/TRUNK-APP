window.loc_onload = function() {
		...
		$(".J-min").click(function(){
		 ...
		 $("#jsonmap1").jqGrid({
			url:$.ctx+'/api/message/personNotice/queryPage',
            datatype: "json",
            postData: {
    			/*"announcementDetail" : $("#txt_name").val(),
    			"announcementName" : $("#txt_name").val()*/
    		},
            colNames:['通知名称','发布时间','通知内容','通知id','阅读状态','名称无格式'],
            colModel:[
                {name:'noticeName',index:'noticeName', width:30, sortable:true,frozen : true ,align:"center",formatter:alarm},//frozen : true固定列 ,formatter:alarm
                {name:'noticeSendTime',index:'noticeSendTime', width:60, align:"center",formatter:formatDate},
                {name:'noticeDetail',index:'noticeDetail', align:"center",hidden:true},
                {name:'noticeId',index:'noticeId', align:"center",hidden:true},
                {name:'readStatus',index:'readStatus', align:"center",hidden:true},
                {name:'noticeName',index:'noticeNameNull', align:"center",hidden:true},
            ],
            rowNum:10,
            rowList:[10,20,30],
            pager: '#pjmap1',//分页的id
            ...
            onSelectRow:function(rowid){//选中行时操作
            	...
            	if (a.readStatus == 2) {
            		$.commAjax({
            			type: 'POST',
            			url: $.ctx+'/api/message/personNotice/read',
            			postData : {
            				"noticeId": a.noticeId
            			},
            			onSuccess: function(){
            	...
            	$("#releaseDate_right").text(a.noticeSendTime);
            	$("#announcementDetail_right").text(a.noticeDetail);
            },
		 ...
		 function formatDate(cellvalue, options, rowObject){
	            return getSmpFormatDateByLong(rowObject.noticeSendTime,true);
	        }
        function alarm(cellvalue, options, rowObject){
           // var html='<a><em class="label-store ui-news-alarm"></em>' +rowObject.announcementName+ '</a>';
        	...
        	if (rowObject.readStatus == 2) {
        		html = '<span class="info-bold">'+rowObject.noticeName+'</span>';
        	} else if (rowObject[4] == 1){
        		html = rowObject[5];
        	} else {
        		html = rowObject.noticeName ; 
        	}
            return html;
        }
	}

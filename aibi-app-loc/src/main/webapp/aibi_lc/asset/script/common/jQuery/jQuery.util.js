...
$.fn.extend({
	...
	mask : function(options) {
		...
		options = $.extend({
			autoShow	: true,
			id			: 'massk',
			left		: $self.offset().left+ parseInt(($self.css('padding-left') || 0).replace('px', '')),
			top			: $self.offset().top + parseInt(($self.css('padding-top') || 0).replace('px', '')),
			width		: $self.width() + 2, // 宽度
			height		: $self.height() + 2, // 高度
			message		: '数据加载中', // 提示内容
			showMessage	: true      // 提示内容
		}, options);
		this.init = function() {
			$mask = $('<div class="window-mask"></div>').attr('id',options.id).appendTo(this);
			$mask.css({
				top				: options.top,
				left			: options.left,
				zIndex			: 99998,

...
public class MD5Util {
	...
	public static String getFileMD5String(File file) {
		...
		try {
			...
			byte[] buffer = new byte[BINARY_1024];
			int numRead = 0;
			while ((numRead = fis.read(buffer)) > 0) {
				messagedigest.update(buffer, 0, numRead);
			}
		} catch (Exception e) {
			LogUtil.error("获取文件MD5校验码失败", e);
		} finally {
			if (fis != null) {
				try {
					fis.close();
				} catch (IOException e) {
				    LogUtil.error("关闭文件流异常", e);
				}
			}
		}
	...
	private static String bufferToHex(byte bytes[], int m, int n) {
	    StringBuilder stringBuilder = new StringBuilder(BINARY * n);
		int k = m + n;
		for (int l = m; l < k; l++) {
	...
	private static void appendHexPair(byte bt, StringBuilder stringBuilder) {
		...
		char c0 = hexDigits[(bt & 0xf0) >> (BINARY*BINARY)];
		// 取字节中低 4 位的数字转换
